---
# Basic info
title: "Zuverlässigkeit und Fehlertoleranz"
date: 2020-07-08
draft: false
type: docs # page type
authors: ["admin"]
...
- **Fehlerbereich $B \subset S$** 
  ...
  - Bsp:
    $$
    S=K_{1} \wedge\left(K_{2} \vee K_{3}\right) \rightarrow B_{1}=\left\{K_{2}\right\} \text { und } B_{2}=\left\{K_{3}\right\}
    $$
...
- **Überlebenswahrscheinlichkeit (component reliability) $R(t)$**
  ...
  $$
  R(t) = \frac{N_s(t)}{N} \quad(=\frac{\text{#fehlerfreie Komponente}}{\text{#Komponente}})
  $$
  ...
  ​	folgt
  $$
  F_{L}(t)=\frac{N_{f}(\mathrm{t})}{\mathrm{N}}=\frac{N-N_s(t)}{N}=1-\mathrm{R}(\mathrm{t}) \label{eq:F_L(t)_R(t)}\\
  
  \Leftrightarrow R(t) = 1 - F_L(t)
  $$
...
- **Ausfallrate $z(t)$**
  ...
  - Ist nur die Ausfallrate bekannt, so ergibt sich die Fehlerwahrscheinlichkeit $F_L(t)$ aus der Anfangswertaufgabe
    $$
    \frac{d}{dt}F_L(t) = f_L(t) \overset{\eqref{eq:ausfallrate}}{=} z(t) \cdot R(t) \overset{\eqref{eq:F_L(t)_R(t)}}{=} z(t)(1-F_L(t))
    $$
    mit der Anfangsbedingung $F_L(0)=0$

    - Die Anfangswertaufgabe hat die Lösung:
      $$
      \mathrm{F}_{\mathrm{L}}(\mathrm{t})=1-\mathrm{e}^{-\int_{0}^{\mathrm{t}} \mathrm{z}(\mathrm{s}) \mathrm{d} \mathrm{s}}
      $$
...
- **Funktionswahrscheinlichkeit $\varphi$**
  ...
  - Für Seriensystem und Parallelsystem
    ...
    - Parallelsystem
      $$
      \varphi\left(\vee_{K \in \Lambda}\right)=\sum_{\varnothing \neq A \in \Lambda}(-1)^{1+\# A} \cdot \varphi\left(\wedge_{K \in A} K\right)
      $$
...
- Seriensystem
  ...
  - Zuverlässigkeit:
    $$
    \varphi\left(\mathrm{S}_{\mathrm{n}}, \mathrm{t}\right)=\prod_{\mathrm{i}=1}^{\mathrm{n}} \varphi\left(\mathrm{K}_{\mathrm{i}}, \mathrm{t}\right)
    $$
